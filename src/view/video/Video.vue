          ...
          >
            ...
            >
              <VideoPlayer
                ...
                ref="videoPlayerComponent1"
                :videoSrc="`${$serverUrl}/video/${this.addFormData.newUrl}`"
                :autoplay="false"
                :key="addFormData.newUrl"
              />
            </el-form-item>
          ...
          <div class="content-expand">
            <ul>
                ...
                <li class="header">
                  <el-col :span="8" type="flex" justify="center">
                    <span
                      ...
                      class="pro-name"
                      ><el-tag style="font-size: 16px">视频播放</el-tag></span
                    >
                  </el-col>
                  <el-col :span="3">
              ...
              <el-divider>
                ...
                >
                  <i class="el-icon-caret-bottom">视频播放</i>
                </div>
                <div
                  v-if="scope.row.path === null"
                ...
                <el-col :span="16">
                  <div class="pro-name">
                    <VideoPlayer
                      ...
                      :videoSrc="
                        scope.row.path
                          ? `${$serverUrl}/video/${scope.row.path}`
                          : ''
                      "
                      :autoplay="false"
                      @video-error="handleVideoError(scope.row)"
                    />
                  </div>
                </el-col>
                <el-col :span="4"
...
// 导入视频播放按钮
import VideoPlayer from "../../components/PlayVideo.vue";
//导入视频上传组件
...
export default {
  ...
  methods: {
    ...
    cancleDialog(done) {
      this.$confirm("关闭后不会保存刚才的数据,确定离开吗？")
        .then((_) => {
          ...
          // 获取视频播放器实例并暂停视频
          if (this.$refs.videoPlayerComponent1) {
            const videoPlayer = this.$refs.videoPlayerComponent1.player;
            if (videoPlayer) {
    ...
    handleUpdate() {
      this.$nextTick(() => {
        // 在组件完全渲染后重新初始化播放器
        const videoPlayer = this.$refs.videoPlayerComponent1;
        if (videoPlayer) {
          videoPlayer.load(); // 重新加载视频播放器
        }
      });
      ...
      console.log(this.addFormData);
       ...
       this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          // 执行保存逻辑
          this.$http
            .put("/videos/update/" + this.addFormData.id, this.addFormData)
            .then((res) => {
              if (res.data.code === 200) {
                this.$message.success(res.data.message);
                this.addFormVisible = false;
    ...
    handleVideoError(row) {
      // 视频加载错误时设置 videoError 为 true
      row.path = null;
    },
    ...
    handleUploadSuccess(uploadedUrl) {
      this.addFormData.newUrl = uploadedUrl; // 设置上传完成后的视频地址
      this.$nextTick(() => {
